...
def time_param_(begin_datetime, end_datetime):
    return (
        f'<timeParam>'
        f'{begin_datetime_(begin_datetime) if begin_datetime is not None else ""}'
        f'{end_datetime_(end_datetime) if end_datetime is not None else ""}'
        f'</timeParam>'
    )
...
def criteria_(method_called, location_param, variable_param, begin_datetime, end_datetime):
    return (
        ...
        f'{location_param_(location_param) if location_param is not None else ""}'
        f'{variable_param_(variable_param) if variable_param is not None else ""}'
...
def query_info_(creation_time, query_url, method_called, location_param, variable_param, begin_datetime, end_datetime):
    return (
        f'<queryInfo>'
        f'{creation_time_(creation_time) if creation_time is not None else ""}'
        f'{query_url_(query_url) if query_url is not None else ""}'
...
def site_info_(site_code, site_name, latitude, longitude, elevation_m, vertical_datum):
    return (
        f'<siteInfo>'
        f'{site_name_(site_name) if site_name is not None else ""}'
        f'{site_code_(site_code) if site_code is not None else ""}'
        f'{geolocation_(latitude, longitude) if all((latitude, longitude)) else ""}'
        f'{elevation_m_(elevation_m) if elevation_m is not None else ""}'
        f'{vertical_datum_(vertical_datum) if vertical_datum is not None else ""}'
        f'</siteInfo>'
    )
...
def unit_(unit_name, unit_abbreviation, unit_code):
    return (
        f'<unit>'
        f'{unit_name_(unit_name) if unit_name is not None else ""}'
        f'{unit_abbreviation_(unit_abbreviation) if unit_abbreviation is not None else ""}'
...
def variable_(variable_code, variable_name, variable_description, unit_name, unit_abbreviation, unit_code, no_data_value):
    return (
        f'<variable>'
        f'{variable_code_(variable_code) if variable_code is not None else ""}'
        f'{variable_name_(variable_name) if variable_name is not None else ""}'
        f'{variable_description_(variable_description) if variable_description is not None else ""}'
        f'{unit_(unit_name, unit_abbreviation, unit_code) if any((unit_name, unit_abbreviation, unit_code)) else ""}'
        f'{no_data_value_(no_data_value) if no_data_value is not None else ""}'
        f'</variable>'
    )
...
def method_(method_code, method_description, method_link):
    return (
        f'<method>'
        f'{method_code_(method_code) if method_code is not None else ""}'
        f'{method_description_(method_description) if method_description is not None else ""}'
        f'{method_link_(method_link) if method_link is not None else ""}'
        f'</method>'
    )
...
def contact_information_(contact_name, type_of_contact, phone, email, address):
    return (
        ...
        f'{contact_name_(contact_name) if contact_name is not None else ""}'
        f'{type_of_contact_(type_of_contact) if type_of_contact is not None else ""}'
        f'{email_(email) if email is not None else ""}'
        f'{phone_(phone) if phone is not None else ""}'
        f'{address_(address) if address is not None else ""}'
        f'</contactInformation>'
    )
...
def source_(source_code, organization, source_description, contact_name, type_of_contact, phone, email, address, source_link):
    return(
        f'<source>'
        f'{source_code_(source_code) if source_code is not None else ""}'
        f'{organization_(organization) if organization is not None else ""}'
        f'{source_description_(source_description) if source_description is not None else ""}'
        ...
        f'{source_link_(source_link) if source_link is not None else ""}'
        f'</source>'
    )
...
def series_(value_count, begin_datetime, end_datetime, variable_code, variable_name, variable_description, unit_name, unit_abbreviation, unit_code, no_data_value, method_code, method_description, method_link, source_code, organization, source_description, contact_name, type_of_contact, phone, email, address, source_link):
    return (
        ...
        f'{value_count_(value_count) if value_count is not None else None}'
